...
BPMNCallActivity.prototype.emitTokens = function(currentProcess, data, createBPMNProcess, returningFromCalledProcess) {
    ...
    if (returningFromCalledProcess) {
        ...
        var calledProcess = currentCallActivity.createCalledProcess(
            calledProcessId,
            currentProcess,
            createBPMNProcess
        );
        ...
        if (startEvents.length === 1) {
            calledProcess.sendEvent(startEvents[0].name);
        } else {
 ...
 * @return {BPMNProcess}
 */
BPMNCallActivity.prototype.createCalledProcess = function(calledProcessId, currentProcess, createBPMNProcess) {
