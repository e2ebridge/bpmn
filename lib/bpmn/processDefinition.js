 ...
 */
exports.getBPMNProcessDefinition = function(flowObject) {
...
/**
 */
BPMNProcessDefinition.prototype.attachBoundaryEvents = function() {
    var self = this;
    var boundaryEvents = this.getBoundaryEvents();
    boundaryEvents.forEach(function(boundaryEvent) {
        var activity = self.getFlowObject(boundaryEvent.attachedToRef);
        if (activity) {
            if (activity.waitForTaskDoneEvent) {
                activity.addBoundaryEvent(boundaryEvent);
            } else {
                // TODO: test
...
BPMNProcessDefinition.prototype.getOutgoingSequenceFlows = function(flowObject) {
    if (!this.sequenceFlowBySourceIndex) {
        this.sequenceFlowBySourceIndex = this.buildSequenceFlowBySourceIndex();
    }
 ...
 * @return {Object}
 */
BPMNProcessDefinition.prototype.buildSequenceFlowBySourceIndex = function() {
    var index = {};
    this.sequenceFlows.forEach(function(sequenceFlow) {
        var entry = index[sequenceFlow.sourceRef];
        if (entry) {
            entry.push(sequenceFlow);
        } else {
            index[sequenceFlow.sourceRef] = [sequenceFlow];
        }
    });
    return index;
};
