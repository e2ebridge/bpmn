...
exports.callHandler = function(name, process, data, handlerDoneCallback) {
    ...
    if (handler) {
        ...
        if (handlerType === 'function') {
            ...
            process._callDefaultEventHandler(eventType, null, mapName2HandlerName(name), "Unknown handler type: '" + handlerType + "'", done);
        }
     } else {
        process._callDefaultEventHandler(eventType, null, mapName2HandlerName(name), "No handler found", done);
    }
