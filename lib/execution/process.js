...
var getTimeoutHandlerPostfix = handlerNameSeparator + "getTimeout";
 ...
 * The doneLoading and doneSaving handler make sense only if we have a persistencyPath.
 ...
 * @param {Function=} doneLoading Called after a process has been loaded and BEFORE deferred events are emitted.
 * @param {Function=} doneSaving Called after a process has been saved and BEFORE deferred events are emitted.
 * @return {BPMNProcessClient}
 */
exports.createBPMNProcess = function(processId, bpmnFilePath, persistencyPath, doneLoading, doneSaving) {
    ...
    handler.doneLoadingHandler = doneLoading || handler.doneLoadingHandler;
    handler.doneSavingHandler = doneSaving || handler.doneSavingHandler;
...
/**
 * Internal creation method exported for testing purpose only.
 * @param {String} id
 ...
 */
exports._createBPMNProcess = createBPMNProcess = function(id, processDefinition, eventHandler, persistency, parentProcess, parentToken, parentHistory) {
    ...
    outgoingSequenceFlows.forEach(function(outgoingSequenceFlow){
       ...
       if (isDiverging) {
            if (outgoingSequenceFlow.name) {
                var handlerName = currentGateway.name + handlerNameSeparator + outgoingSequenceFlow.name;
...
BPMNProcess.prototype._emitTokensAtCallActivity = function(currentCallActivity, data, returningFromSubprocess) {
    ...
    if (returningFromSubprocess) {
        ...
        var currentHistoryEntry = this.history.getLastEntry(callActivityName);
        ...
        var subprocess = createBPMNProcess(subprocessId, processDefinition, handler,
            this.persistency, this, callActivityToken, currentHistoryEntry.subprocessHistory);
