...
exports.getBPMNProcess = function(processId, bpmnFilePath, persistencyPath, done) {
    ...
    var handler = getHandler(bpmnFilePath);
    var processDefinition = bpmnProcessDefinitionModule.getBPMNProcessDefinition(bpmnFilePath);
    var bpmnProcess = createBPMNProcess(processId, processDefinition, handler, persistency, done);
 ...
 * @constructor
 */
exports.createBPMNProcess = createBPMNProcess = function(id, processDefinition, eventHandler, persistency, done, parentProcess, parentToken) {
    var bpmnProcess = activeProcessesCache[id];
    if (!bpmnProcess) {
        bpmnProcess = new BPMNProcess(id, processDefinition, eventHandler, persistency, done, parentProcess, parentToken);
 ...
 * @constructor
 */
function BPMNProcess(id, processDefinition, eventHandler, persistency, done, parentProcess, parentToken) {
    ...
    this.processId = this.processDefinition.name + "::" + id;
    this.deferredEvents = [];
    this.activeTimers = {};
...
BPMNProcess.prototype.loadState = function() {
    if (this.persistency) {
        ...
        var doneLoading = function(error, loadedData) {
            ...
            if (this.doneLoading) {
                this.doneLoading.call(self.processClient, error, doneLoading);
            }
...
BPMNProcess.prototype._emitTokensAtCallActivity = function(currentCallActivity, data, returningFromSubprocess) {
    ...
    if (returningFromSubprocess) {
        ...
        var subprocessId = this.processId + "::" + currentCallActivity.name;
        var bpmnFilePath = currentCallActivity.location;
        var handlerName = mapName2HandlerName(currentCallActivity.name);
        var mockupHandler = this._getHandler(handlerName);
        var handler = mockupHandler && typeof mockupHandler === "object" ? mockupHandler : getHandler(bpmnFilePath);
        var processDefinition = bpmnProcessDefinitionModule.getBPMNProcessDefinition(bpmnFilePath);
        var currentToken = state.createTokenAt(currentCallActivity.name, this.processId);
        var subprocess = createBPMNProcess(subprocessId, processDefinition, handler, this.persistency, this.doneLoading, this, currentToken);
        currentToken.substate = subprocess.getState();
    ...
    self.onActivityFinished(function(activityName, data) {
        ...
        if (numberOfTokensAtActivity) {
            ...
            if (hasOutgoingFlows) {
                ...
                currentProcess._callHandler(activityName + activityFinishedHandlerPostfix, data, activityFinishedHandlerIsDone);
            } else {
                // TODO: I think we should enhance the default event hanlder by a reason string/code. E.g. noTokenAt, notOutgoingFlow, etc.
 ...
 * @type {Object}
 */
exports.getHandler = getHandler = function(bpmnFilePath) {
