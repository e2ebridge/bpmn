...
BPMNProcess.prototype._emitNextTokens = function(currentFlowObject, data) {
    ...
    if (currentFlowObject.isExclusiveGateway) {
        ...
        self._emitTokensAtCallActivity(currentFlowObject, data);
    } else {
 ...
 * @private
 */
BPMNProcess.prototype._emitTokensAtCallActivity = function(currentCallActivity, data) {
    var self = this;
    var state = self.state;

    state.createTokenAt(currentCallActivity);

   var subprocessId = this.processInstanceId + "::" + currentCallActivity.name;
   var subprocess = this._getBPMNSubprocess(subprocessId, currentCallActivity.location);
   var processDefinition = subprocess.getProcessDefinition();
 ...
 * @return {BPMNProcessClient}
 */
BPMNProcess.prototype._getBPMNSubprocess = function(processId, bpmnFilePath, done) {

    var handler = getHandler(bpmnFilePath);
    var processDefinition = bpmnProcessDefinitionModule.getBPMNProcessDefinition(bpmnFilePath);

    var bpmnProcess = new BPMNProcess(processId, processDefinition, handler, this.persistency);
    bpmnProcess.loadState(done);
