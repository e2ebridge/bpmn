 ...
 */
exports.parse = function(fileName, errorQueue) {
        ...
        if (inBPMN2Namespace) {
            ...
            if (currentCollaborationDefinition) {
                if (messageFlows.isMessageFlowName(localName)) {
                    ...
                    currentCollaborationElement = participant.createBPMNParticipant(node, fileName);
            ...
            if (processDefinition.isProcessName(localName)) {
                if (processDefinition.isExecutable(node)) {
                    ...
                    if (!currentProcessDefinition.name) {
                        var baseFileName = fileUtils.removeFileExtension(path.basename(fileName));
                        currentProcessDefinition.name = utils.toUpperCamelCase(baseFileName);
                    }
