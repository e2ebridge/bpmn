...
BPMNProcess.prototype.sendEvent = function(eventName, data) {
    ...
    if (flowObject) {
        if (flowObject.isStartEvent) {
            ...
            var activity = self.processDefinition.getFlowObject(flowObject.attachedToRef);
            ...
            if (numberOfTokensAtActivity) {
                ...
                self._putTokenAt(flowObject.name, data);
                 ...
                 throw new Error("The process '" + processDefinition.name + "' cannot catch the boundary event '" + eventName + "' because the activity '" + activity.name + "' doesn't expect one.");
            }
        } else {
