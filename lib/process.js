...
BPMNProcess.prototype._emitTokenAtExclusiveGateway = function(currentGateway, data) {
    ...
    currentGateway.outgoingRefs.forEach(function(outgoingRef){
        ...
        var outgoingSequenceFlow = self.processDefinition.getOutgoingSequenceFlow(outgoingRef);
        if (isDiverging) {
...
BPMNProcess.prototype._registerBoundaryEvents = function(currentActivity) {
    var self = this;
    currentActivity.boundaryEvents.forEach(function(boundaryEvent) {
...
BPMNProcess.prototype._setTimerOn = function(timerBoundaryEvent) {
    ...
    if (isNaN(timeout)) {
        ...
        throw Error("The getTimeout handler '" + "' does not return a number");
    } else {
        timer = setTimeout(timeoutHappened, timeout);
...
BPMNProcess.prototype._unregisterBoundaryEvents = function(currentFlowObject) {
    ...
    if (activity.hasBoundaryEvents) {
        activity.boundaryEvents.forEach(function(boundaryEvent) {
            if (boundaryEvent.isTimerEvent) {
...
BPMNProcess.prototype._registerTaskDoneEvent = function() {
    ...
    self.onTaskDone(function(taskName, data) {
        if (this._acceptTaskDoneEvent(taskName)) {
            var taskDoneHandlerIsDone = function(data) {
                var currentTask = self.processDefinition.getFlowObjectByName(taskName);
                self._emitNextTokens(currentTask, data);
            };
            self._callHandler(taskName + taskDoneHandlerPostfix, data, taskDoneHandlerIsDone);
        } else {
    ...
    self.onTokenArrivedEvent(function(currentFlowObjectName, data) {
        ...
        var handlerDone = function(data) {
            if (currentFlowObject.waitForTaskDoneEvent) {
                ...
                self.persist();
                if (currentFlowObject.hasBoundaryEvents) {
