    ...
    if (this.persistency) {
        ...
        var doneLoading = function(error, loadedData) {
            if (error) {
                ...
                if (loadedData) {
                    loadPersistedData(self, loadedData);
                }
            }
            if (error || loadedData) {
...
function loadPersistedData(currentProcess, loadedData) {
    var state = new BPMNProcessState(loadedData.state);
    var history = new BPMNProcessHistory(loadedData.history);
    var pendingTimeouts = new BPMNPendingTimeouts(currentProcess);
    pendingTimeouts.addTimeouts(loadedData.eventName2TimeoutMap);

    currentProcess.history = history;
    currentProcess.state = state;
    currentProcess.pendingTimeouts = pendingTimeouts;
    currentProcess.data = loadedData.data || {};

    var callActivityTokens = state.findCallActivityTokens();
    callActivityTokens.forEach(function(callActivityToken) {
        ...
        callActivity.createCalledProcess(
