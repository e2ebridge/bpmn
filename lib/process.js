    ...
    self.on(activityEndEvent, function(activityName, data) {
        ...
        if (numberOfTokensAtActivity) {
            ...
            if (hasOutgoingFlows) {
                ...
                self.callDefaultEventHandler(activityEndEvent, activityName, handlerName, "Found no outgoing flow.");
            }
        } else {
...
BPMNProcess.prototype._registerThrowIntermediateEvents = function() {
    ...
    self.on(throwIntermediateEvent, function(eventName, data) {
        ...
        var tokensAtCatchIntermediateEvent = self.state.findTokens(eventName);
        var numberOfTokensAtCatchIntermediateEvent = tokensAtCatchIntermediateEvent.length;
        if (numberOfTokensAtCatchIntermediateEvent) {
            var catchIntermediateEventObject = self.processDefinition.getFlowObjectByName(eventName);
            var outgoingFlows = self.processDefinition.getOutgoingSequenceFlows(catchIntermediateEventObject);
            var hasOutgoingFlows = outgoingFlows.length > 0;
            if (hasOutgoingFlows) {
                ...
                self.callDefaultEventHandler(throwIntermediateEvent, eventName, handlerName, "Found no outgoing flow.");
            }
        } else {
...
BPMNProcess.prototype._registerThrowBoundaryEvents = function() {
    // TODO: we do something similar for timeouts. We should re-use this code!
    var self = this;
