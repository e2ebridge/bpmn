...
var finishedActivityEvent = "finishedActivityEvent";
...
function BPMNProcess(id, processDefinition, eventHandler, persistency, parentProcess, parentToken, parentHistoryEntry) {
    ...
    this._registerFinishingEvents(finishedActivityEvent, activityModule.activityFinishedHandlerPostfix);
    this._registerFinishingEvents(throwIntermediateEvent);
    this._registerBoundaryEvents();
}
...
BPMNProcess.prototype.sendEvent = function(eventName, data) {
    ...
    if (flowObject) {
        if (flowObject.isStartEvent) {
            ...
            // The token must be already on the intermediate catch event, otherwise we
            // would get an error later on.
            process.nextTick(function() {
                // TODO: document why we need nextTick
...
BPMNProcess.prototype.taskDone = function(taskName, data) {
    this.emitActivityFinishedEvent(taskName, data);
};
...
BPMNProcess.prototype.emitActivityFinishedEvent = function(eventName, data) {
    this._emitEvent(finishedActivityEvent, eventName, data);
};
 ...
 * @private
 */
BPMNProcess.prototype._registerFinishingEvents = function(eventType, handlerPostfix) {
    ...
    self.on(eventType, function(activityName, data) {
        ...
        if (numberOfTokensAtActivity) {
            ...
            if (hasOutgoingFlows) {
                ...
                self.callDefaultEventHandler(eventType, activityName, handlerName, "Found no outgoing flow.");
            }
        } else {
            self.callDefaultEventHandler(eventType, activityName, handlerName, "Process cannot handle this activity because it is not currently executed.");
        }
    });
};
 ...
 * @private
 */
BPMNProcess.prototype._registerBoundaryEvents = function() {
    var self = this;
    self.on(throwBoundaryEvent, function( eventName, data) {
        var handlerName = handlerModule.mapName2HandlerName(eventName);
        var catchBoundaryEvent = self.processDefinition.getFlowObjectByName(eventName);
        var activity = self.processDefinition.getFlowObject(catchBoundaryEvent.attachedToRef);
        var tokensAtActivity = self.state.findTokens(activity.name);
        var numberOfTokensAtActivity = tokensAtActivity.length;
        if (numberOfTokensAtActivity) {
            self.state.removeTokenAt(activity);
            self._putTokenAt(catchBoundaryEvent.name, data);
        } else {
