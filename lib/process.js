...
BPMNProcess.prototype._removeTimeout = function(timeoutEvent) {
    var pendingTimeout = this.pendingTimeouts[timeoutEvent];
    if (pendingTimeout) {
        clearInterval(this.timeoutChecker[timeoutEvent]);
        delete this.timeoutChecker[timeoutEvent];
        delete this.pendingTimeouts[timeoutEvent];
    }
};
 ...
 * @private
 */
BPMNProcess.prototype._addTimeout = function(timerBoundaryEvent) {
    var self = this;

    var timeEventName = timerBoundaryEvent.name;
    var getTimeoutHandlerName = timeEventName + getTimeoutHandlerPostfix;
    ...
    if (isNaN(timeout)) {
        ...
        var checkTimeout = function() {
            /** {BPMNActivityTimeout} */
            var pendingTimeout = self.pendingTimeouts[timeEventName];
            ...
            if (pendingTimeout) {
                ...
                if (now > pendingTimeout.at) {
                    ...
                    self._removeTimeout(timeEventName);
                    self._putTokenAt(pendingTimeout.eventName);
                }
            }
        };

        //console.log("Set timeout for '" + timeEventName + "' timeout=" + timeout);
        self.pendingTimeouts[timeEventName] = new ActivityTimeout(timeEventName, timeout, true);
        self.timeoutChecker[timeEventName] = setInterval(checkTimeout, 250);
    }
};
 ...
 * @constructor
 */
function ActivityTimeout(timeEventName, timeout, isRelative) {
    var at, now;
        ...
        if (flowObject.isStartEvent) {
            ...
            if (flowObject.isIntermediateCatchEvent) {
                process.nextTick(function() {
                   self._emitEvent(intermediateEvent, eventName, data);
                });
            } else {
...
BPMNProcess.prototype._registerBoundaryEvents = function(currentActivity) {
    ...
    boundaryEvents.forEach(function(boundaryEvent) {
        if (boundaryEvent.isTimerEvent) {
            self._addTimeout(boundaryEvent);
        }
    });
};
...
BPMNProcess.prototype._unregisterBoundaryEvents = function(currentFlowObject) {
    ...
    boundaryEvents.forEach(function(boundaryEvent) {
        if (boundaryEvent.isTimerEvent) {
            // when leaving the activity we get rid of the active timeout
            self._removeTimeout(boundaryEvent.name);
        }
    });
};
 ...
 * @param {Function=} done
 */
function logDefaultedErrors(error, done) {
