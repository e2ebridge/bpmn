 ...
 * @return {BPMNProcess}
 */
function createBPMNProcess(id, processDefinition, eventHandler, persistency, parentProcess, parentToken) {
    var bpmnProcess = new BPMNProcess(id, processDefinition, eventHandler, persistency, parentProcess, parentToken);
...
function BPMNProcess(id, processDefinition, eventHandler, persistency, parentProcess, parentToken) {
    ...
    if (parentToken) {
        this.history = new BPMNProcessHistory(parentToken.subhistory);
        parentToken.subhistory = this.history;
...
BPMNProcess.prototype.persist = function() {
    if (this.persistency) {
        ...
        var persistentData = {
            ...
            parentToken: this.parentToken || null,
            data: this.data,
            state: this.state,
            history: this.history,
            eventName2TimeoutMap: this.pendingTimeouts.eventName2TimeoutMap
        };
...
BPMNProcess.prototype.loadPersistedData = function() {
    if (this.persistency) {
        ...
        var doneLoading = function(error, loadedData) {
            if (loadedData) {
                self.setPersistedData(loadedData);

                // TODO: do we have to find ALL call activity tokens, i.e. traversing all levels?
