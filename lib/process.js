...
function BPMNProcess(id, processDefinition, eventHandler, persistency, parentProcess, parentToken, parentHistory) {
    ...
    this._registerExternalEvents(activityFinishedEvent, activityModule.activityFinishedHandlerPostfix);
    this._registerExternalEvents(externalEvent);
}
...
/**
 * TODO: just one send event - get rid of send start event
 * @param {String} eventName
 * @param {Object} data
 */
BPMNProcess.prototype.sendEvent = function(eventName, data) {
    this._emitEvent(externalEvent, eventName, data);
};
 ...
 * @private
 */
BPMNProcess.prototype._registerExternalEvents = function(eventType, handlerPostfix) {
